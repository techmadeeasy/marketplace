<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Dashboard') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="mb-5">
                        <a href="{{ route('dashboard.listings.create') }}" class="bg-blue-500 hover:bg-blue-600 mt-4 lg:mt-0 lg:ml-3 rounded-full text-xs font-semibold text-white uppercase py-3 px-8">Create new listing</a>
                        <a href="{{ route('dashboard.listings.edit', [$listing->id]) }}" class="bg-gray-100 hover:bg-blue-600 mt-4 lg:mt-0 lg:ml-3 rounded-full text-xs font-semibold text-black uppercase py-3 px-8">Edit this listing</a>
                    </div>
                    <article class="max-w-4xl mx-auto lg:grid lg:grid-cols-12 gap-x-10">
                        <div class="col-span-4 lg:text-center lg:pt-14 mb-10">
                            <img src="{{ $listing->image_path }}" alt="" class="rounded-xl">

                            <p class="mt-4 block text-gray-400 text-xs">
                                Published <time>{{ $listing->created_at->diffForhumans() }}</time>
                            </p>

                            <div class="flex items-center lg:justify-center text-sm mt-4">
                                <div class="ml-3 text-left">
                                    <h5 class="font-bold">{{ $listing->user->name }} </h5>
                                </div>
                            </div>
                        </div>

                        <div class="col-span-8">
                            <div class="hidden lg:flex justify-between mb-6">
                                <a href="{{ route('dashboard.listings.index') }}" class="transition-colors duration-300 relative inline-flex items-center text-lg hover:text-blue-500">
                                    <svg width="22" height="22" viewBox="0 0 22 22" class="mr-2">
                                        <g fill="none" fill-rule="evenodd">
                                            <path stroke="#000" stroke-opacity=".012" stroke-width=".5" d="M21 1v20.16H.84V1z">
                                            </path>
                                            <path class="fill-current" d="M13.854 7.224l-3.847 3.856 3.847 3.856-1.184 1.184-5.04-5.04 5.04-5.04z">
                                            </path>
                                        </g>
                                    </svg>

                                    Back to Posts
                                </a>

                                <div class="space-x-2">
                                    <a href="#" class="px-3 py-1 border border-blue-300 rounded-full text-blue-300 text-xs uppercase font-semibold" style="font-size: 10px">
                                        {{ $listing->category->title }}
                                    </a>
                                </div>
                            </div>

                            <h1 class="font-bold text-3xl lg:text-4xl mb-10">
                                {{ $listing->title }}
                            </h1>

                            <div class="space-y-4 lg:text-lg leading-loose">
                                {{ $listing->description }}
                            </div>
                            <p>
                                {{ $listing->currency }} {{ number_format($listing->price, '2', '.', ' ') }}
                            </p>
                        </div>
                    </article>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>
